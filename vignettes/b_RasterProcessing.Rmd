...
```{r temperature processing, eval=FALSE}
...
update(temp, strip  = strip.custom(strip.levels = TRUE,
                             ...
                             bg = "black",
                             fg = "white",
                             par.strip.text = list(col = "white", cex = 1, font = 2)))
```

```{r temperature processing hidden, echo=FALSE}
#Creating a bottom raster from the point shapefile
temperatureBottom <- bottomRaster(temperature)
...
oxygen <- sf::st_read(paste0(td, "/oxygen/woa18_all_A00mn01_cropped.shp"))
oxygen <- as(oxygen, "Spatial")
...
# Creating a SpatRaster vector
template <- rast(nrows = length(unique(oxygen@coords[,2])),
                 ncols = length(unique(oxygen@coords[,1])),
                 extent = ext(oxygen))
oxygen <- rasterize(x = oxygen@coords, y = template,
                   values = oxygen@data)
...
update(temp, strip  = strip.custom(strip.levels = TRUE,
                             ...
                             bg = "black",
                             fg = "white",
                             par.strip.text = list(col = "white", cex = .8, font = 2)))
```

```{r AOU plot, echo=FALSE, out.width = '100%', out.height= '100%'}
