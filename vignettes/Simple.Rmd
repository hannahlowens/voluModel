...
knitr::opts_chunk$set(echo = TRUE, error = TRUE, dpi=300,fig.width=5)
knitr::opts_knit$set(root.dir = system.file('extdata/', package='voluModel'))
load(system.file("extdata/datForVignette.RData", 
                             package='voluModel'))
```
...
```{r environmental data loading temperature, eval=FALSE}
library(rgdal)
library(raster)
library(voluModel)
temperature <- readOGR(system.file("extdata/woa18_decav_t00mn01_shape/woa18_decav_t00mn01.shp", 
                                   package = "voluModel"))
temperature@data[temperature@data == -999.999] <- NA
temperature <- rasterFromXYZ(cbind(temperature@coords,temperature@data))
...
plot(temperature[[c(1,nlayers(temperature))]], 
     main = c("Temperature, surface",
              "Temperature, max depth"))
```

```{r environmental data loading temperature display, echo=FALSE}
library(raster)
library(voluModel)
library(rgdal)
plot(temperature[[1]], main = "Temperature, surface")
```
...
```{r environmental data loading oxygen, eval=FALSE}
library(fields)
oxygen <- readOGR(system.file("extdata/woa18_all_A00mn01_shape/woa18_all_A00mn01.shp",
                              package = "voluModel"))
...
plot(oxygen[[c(1,nlayers(oxygen))]], 
     main = c("Apparent Oxygen Utilization, surface",
              "Apparent Oxygen Utilization, max depth"))
```
```{r environmental data loading oxygen display, echo=FALSE}
plot(oxygen[[1]], main = "Apparent Oxygen Utilization, surface")
```
...
plot(oxygenSmooth[[c(1,nlayers(oxygenSmooth))]], 
     main = c("Apparent Oxygen Utilization, surface",
              "Apparent Oxygen Utilization, max depth"))
```
```{r smooth oxygen display, echo=FALSE}
plot(oxygenSmooth[[1]], main = "Apparent Oxygen Utilization, surface")
```
...
occurrences <- occs[,c("decimalLatitude", "decimalLongitude", "depth")] # Remove metadata
...
rm(indices, layerNames, tempPoints, i, downsampledOccs)
...
It's hard to visualize things in three dimensions, but here's an attempt. This script plots a transparent layer of suitable habitat for each depth coordinate. The redder the color, the shallower the layer, the bluer, the deeper. The more saturated the color, the more layers with suitable habitat. Here, I am plotting suitability from 55m to 1300m, the depth range of occurrences used to train the envelope model.
...
redVal <- 1
blueVal <- 0
stepSize <- 1/(max(indices)-min(indices) + 1)
image(envelopeModel[[min(indices)]], col = c(rgb(0,0,0,0), 
                                  rgb(redVal,0,blueVal,stepSize)),
      main = paste0("Suitability from ", names(envelopeModel)[[min(indices)]], " to ",
                    names(envelopeModel)[[max(indices)]]))
for(i in (min(indices)+1):max(indices)){
  redVal <- redVal - stepSize
  blueVal <- blueVal + stepSize
  image(envelopeModel[[i]], 
        col = c(rgb(0,0,0,0), rgb(redVal,0, blueVal,stepSize)), 
        add = T) 
}

plot(rnaturalearth::ne_countries(scale = "medium", returnclass = "sf")[1], col = "black", add = T)
rm(layerNames, indices, redVal, blueVal, stepSize, i, envelopeModel, occurrences)
```
...
indices <- sort(unique(datForMod$index))

redVal <- 1
blueVal <- 0
stepSize <- 1/(max(indices)-min(indices) + 1)
image(glmThresholded[[min(indices)]], col = c(rgb(0,0,0,0), 
                                              rgb(redVal,0,blueVal,stepSize)),
      main = paste0("Suitability from ", names(glmThresholded)[[min(indices)]], " to ",
                    names(glmThresholded)[[max(indices)]]))
for(i in (min(indices)+1):max(indices)){
  redVal <- redVal - stepSize
  blueVal <- blueVal + stepSize
  image(glmThresholded[[i]], 
        col = c(rgb(0,0,0,0), rgb(redVal,0, blueVal,stepSize)), 
        add = T) 
}

plot(rnaturalearth::ne_countries(scale = "medium", returnclass = "sf")[1], col = "black", add = T)
rm(layerNames, indices, redVal, blueVal, stepSize, i, glmThresholded, datForMod)
```
