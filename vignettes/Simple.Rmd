...
```{r environmental data loading oxygen, eval=T, asis=T}
library(fields)
...
The first step is down-sampling the occurrence data so that there is only one occurrence per voxel (i.e. 3D pixel) of environmental data. We do this to avoid over-fitting the model due to biased sampling. The resampled points are centered in each voxel. You will notice the original and cleaned datasets completely agree. This is because the dataset supplied was already cleaned.
