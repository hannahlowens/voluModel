...
```{r OLD clean occurrence data, eval=T}
...
```{r OLD environmental data loading temperature, eval=T, asis=T}
library(rgdal)
library(raster)
library(voluModel)
td <- tempdir()
...
```{r OLD environmental data loading oxygen, eval=T, asis=T, warning=FALSE}
library(fields)
td <- tempdir()
...
```{r OLD smooth oxygen display, echo=FALSE}
plot(oxygenSmooth[[1]], 
...
```{r OLD downsample to voxel, eval=TRUE, warning=FALSE, message=FALSE}
occurrences <- occsClean[,c("decimalLatitude", "decimalLongitude", "depth")] 
...
```{r OLD environmental background sampling}
