...
Now I will calculate two `rasterBricks`. In the first, `extrapolation`, MESS values zero and below (i.e. signalling extrapolation) will be reclassified as 1, which will allow me to easily map where extrapolation is occurring.
...
```{r reclassify MESS and plot GLM with no extrapolation}
# Reclassify MESS 
...
# Plot MESS
plotLayers(glmThreshNoExtrapolation, land = land, landCol = "black", title = "Areas of suitable habitat with no model extrapolation,\n 0m to 800m")
```
