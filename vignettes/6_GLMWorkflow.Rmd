...
The first step is down-sampling the occurrence data so that there is only one occurrence per voxel (the 3D equivalent of a pixel) of environmental data. We do this to avoid over-fitting the model due to biased sampling. The resampled points are centered in each voxel. You will notice that in this instance there was no re-centering of points, as the original dataset was already downsampled to the resolution of the environmental data. The red dots show coordinates from the original dataset where there were only occurrences with depths of 0 and/or 2100m, which were removed in data cleaning.
...
Then we unit the presence and absence data into a single data object.
...
In the second `RasterBrick`, `noExtrapolation`, I reclassify the layers so that positive MESS values = 1 and MESS values less than or equal to 0 equal 0. This will make plotting cropping out areas of extrapolation from the niche model projection easier. I simply multiply the thresholded GLM map by the `noExtrapolation` `RasterBrick`.
